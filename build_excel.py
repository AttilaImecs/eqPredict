#!/usr/bin/env python3
"""
STEP 3 of 3 -- Assemble the checkpoint CSVs into a formatted Excel workbook.
...
M_MEASURES = [
    "close", "adj_close", "volume", "market_cap_est", "pe_trailing_est",
]
...
def readme_frame(counts):
    rows = [
        ...
        ("", ""),
        ("WHAT THIS IS", ""),
        ("", "5 years of financial data for the union of the S&P 500, all Nasdaq-listed"),
        ("", "common stocks, and the Fortune 500 (public members only)."),
        ("", ""),
        ("SHEETS", ""),
...
def main():
    ...
    uni = pd.read_csv("universe.csv") if os.path.exists("universe.csv") else pd.DataFrame()
    ...
    cov = (
        ...
        .merge(qq.groupby("ticker").agg(
            ...
            quarters_with_ebit=("ebit", "count"),
            quarters_with_net_income=("net_income", "count"),
            latest_quarter=("period_end", "max"),
        ), on="ticker", how="left")
        .merge(m.groupby("ticker").agg(
            months=("month", "count"),
            latest_month=("month", "max"),
        ), on="ticker", how="left")
    )
    for c in ("quarters", "quarters_with_revenue", "quarters_with_ebit",
              "quarters_with_net_income", "months"):
        cov[c] = cov[c].fillna(0).astype(int)
    cov["complete"] = (cov["quarters_with_revenue"] >= 18) & (cov["months"] >= 55)
    ...
    counts = {
        ...
        "Fully complete companies": int(cov["complete"].sum()),
